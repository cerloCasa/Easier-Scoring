            ...
            if scoring_hand[i].debuff then
                ...
                for j=1,#reps do
                    ...
                    for ii = 1, #effects do
                        ...
                        -- If x_chips added, do x_chips add event and add mult the chips to the total
                        if effects[ii].x_chips then
                            if effects[ii].card then juice_card(effects[ii].card) end
                            hand_chips = mod_chips(hand_chips * effects[ii].x_chips)
                            update_hand_text({delay = 0}, {chips = hand_chips})
                            EzSc_card_eval_status_text(scoring_hand[i], 'xChips', effects[ii].x_chips, percent)
                        end
                ...
                while j <= #reps do
                    ...
                    for ii = 1, #effects do
                        ...
                        if effects[ii].chips then
                            ...
                            hand_chips = mod_chips(hand_chips + effects[ii].chips)
                            update_hand_text({delay = 0}, {chips = hand_chips})
                            EzSc_card_eval_status_text(G.hand.cards[i], 'chips', effects[ii].chips, percent)
                        end
                        ...
                        if effects[ii].x_chips then
                            ...
                            hand_chips = mod_chips(hand_chips * effects[ii].x_chips)
                            update_hand_text({delay = 0}, {chips = hand_chips})
                            EzSc_card_eval_status_text(G.hand.cards[i], 'xChips', effects[ii].x_chips, percent)
                        end
                        ...
                        if effects[ii].mult then
                            ...
                            mult = mod_mult(mult + effects[ii].mult)
                            update_hand_text({delay = 0}, {mult = mult})
                            card_eval_status_text(G.hand.cards[i], 'mult', effects[ii].mult, percent)
                        end
